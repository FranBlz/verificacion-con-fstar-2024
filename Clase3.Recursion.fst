...
let rec flip2 (x:int) : Tot int (decreases %[abs(x); x])= // REVISAR, idea: (5, -5) >> (4, 4) >> (4, -4) ...
  admit();
  if x = 0 then 0
...
let rec sum (xs:list int) : int =
  (* Por qué se acepta esta función? Porque es una función total que implicitamente verifica que disminuye sobre xs *)
  if Nil? xs
  then 0
...
let rec sum' (xs:list int) : Tot int =
  (* Por qué *no* se acepta esta función? Termina? REVISAR *)
  if Nil? xs
  then 0
